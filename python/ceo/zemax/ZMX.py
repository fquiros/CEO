...
print(path)
GlassDir = os.path.join(path,"glass")

temp = 20.0
pres = 1.0

class ZemaxModel():
    def __init__(self, filename, field=1):
        #self.src = src
        self.field = field
    ...
    def OBSC(self, *line): pass
    def OBDC(self, x, y):
        self.current["OBDC"] = [float(x), float(y)]
...
class ZmxSurf2CEO:
    ...
    def OBDC(self, xy):
        self.kwargs["conic_origin"] = [xy[1]*1e-3,xy[0]*1e-3,0]
        #self.kwargs["origin"] = [0,0,0]
    ...
    def do_coordbrk(self, parms):
        ...
        self.kwargs["origin"][0] += parms[1]*1e-3
        self.kwargs["origin"][1] += parms[2]*1e-3
